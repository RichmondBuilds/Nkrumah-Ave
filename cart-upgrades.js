/**
 * NKRUMAH AVE — CART UPGRADES PATCH
 * Drop this <script> tag just before </body> in index.html
 * 
 * ✅ 1. Fly-to-cart animation
 * ✅ 2. Image toast notification
 * ✅ 3. Quantity steppers
 * ✅ 4. Sticky checkout bar
 * ✅ 5. Free shipping progress bar (threshold: GHS 200)
 * ✅ 6. Auto-merge duplicates
 * ✅ 7. Save for later
 * ✅ 8. Cart recommendations
 * ✅ 9. Cart shake reminder (45s idle)
 */
...
// ── INJECT STYLES ─────────────────────────────────────────────────────────
const CSS = `
@keyframes _cartBounce {0%{transform:scale(1)}30%{transform:scale(1.35)}60%{transform:scale(.9)}100%{transform:scale(1)}}
@keyframes _cartShake {0%,100%{transform:translateX(0)}15%{transform:translateX(-6px) rotate(-5deg)}30%{transform:translateX(6px) rotate(5deg)}45%{transform:translateX(-5px) rotate(-3deg)}60%{transform:translateX(4px) rotate(2deg)}75%{transform:translateX(-3px) rotate(-1deg)}100%{transform:translateX(0)}}
.cart-fab-bounce{animation:_cartBounce .5s ease!important}
.cart-fab-shake{animation:_cartShake .9s ease!important}

/* Image Toast */
#_imgToast{position:fixed;top:70px;left:50%;transform:translateX(-50%) translateY(-12px);background:#111;border:1px solid #00ff00;border-radius:14px;padding:10px 12px;display:flex;align-items:center;gap:10px;opacity:0;transition:opacity .3s ease,transform .3s ease;pointer-events:none;z-index:200;box-shadow:0 8px 32px rgba(0,255,0,.2)}
...
._tn s{display:block;font-size:.82em;color:#00ff00;font-weight:bold}
...
.qty-btn{width:26px;height:26px;background:#1a1a1a;border:1px solid #2a2a2a;color:#00ff00;border-radius:6px;font-size:1em;font-weight:bold;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .2s}
...
._stbtn{flex:1;padding:13px;background:linear-gradient(90deg,#00ff00,#00cc00);color:#000;border:none;border-radius:10px;font-size:.88em;font-weight:bold;cursor:pointer;text-transform:uppercase;letter-spacing:.5px}
...
stickyBar.innerHTML = `
  <div class="_stp">
    <div class="_stplbl">Total</div>
    <div class="_stpamt" id="_stpamt">GHS 0</div>
  </div>
  <button class="_stbtn" onclick="toggleCart()">🛒 Checkout</button>
`;
...
// ── CART RECS CONTAINER (inject into cart modal) ───────────────────────────────
const cartModal = document.getElementById('cartModal');
if (cartModal) {
  const recDiv = document.createElement('div');
  recDiv.id = '_cartRecsContainer';
  cartModal.querySelector('.modal-content').appendChild(recDiv);
}
...
// ── HELPERS ───────────────────────────────────────────────────────────
function _getImg(productId) {
  const p = (window.products||[]).find(p => p.id === productId);
  return p && p.media && p.media[0] && p.media[0].url ? p.media[0].url : null;
}
function _isShoe(productId) {
  const p = (window.products||[]).find(p => p.id === productId);
  return p && p.category === 'shoes';
}
function _finalPrice(p) {
  return p.discount ? p.price - Math.round(p.price * p.discount / 100) : p.price;
}

// ── 1 & 2: FLY TO CART + IMAGE TOAST ─────────────────────────────────────────
function flyToCart(srcEl, productId) {
  ...
  if (srcEl && fab) {
    ...
    clone.style.cssText = [
      ...
      'border-radius:8px', 'box-shadow:0 4px 20px rgba(0,255,0,.4)',
      'transition:left .55s cubic-bezier(.25,.46,.45,.94),top .55s,width .5s ease,height .5s ease,opacity .4s ease,border-radius .4s ease',
      `left:${sr.left}px`, `top:${sr.top}px`, `width:${sr.width}px`, `height:${sr.height}px`
    ].join(';');
    ...
    setTimeout(() => {
      ...
      setTimeout(() => fab.classList.remove('cart-fab-bounce'), 500);
    }, 600);
  }
  _showToast(p, imgSrc);
}
...
function _updateStickyBar() {
  ...
  amt.textContent = 'GHS ' + base.toLocaleString();
...
function _merge(item) {
  ...
  if (ex) { ex.qty = (ex.qty||1)+1; return false; }
  item.qty = 1; window.cart.push(item); return true;
}
...
function _cartRecs() {
  ...
  return '<div class="_recs"><div class="_recstitle">You might also like</div><div class="_recsrow">'
    + recs.map(p => {
        const f = _finalPrice(p);
        const img = p.media&&p.media[0]&&p.media[0].url ? `<img src="${p.media[0].url}">` : (p.category==='shoes'?'👟':'👕');
        return `<div class="_reccard" onclick="toggleCart();openProductDetail('${p.id}')"><div class="_recimg">${img}</div><div class="_recinfo"><div class="_recname">${p.name}</div><div class="_recprice">GHS ${f.toLocaleString()}</div><button class="_recatc" onclick="event.stopPropagation();window._quickAdd('${p.id}',this)">Add</button></div></div>`;
      }).join('')
    + '</div></div>';
}
...
function _resetIdle() {
  ...
  _idleT = setTimeout(() => {
    if (window.cart && window.cart.length > 0) {
      const fab = document.querySelector('.cart-fab');
      if (fab) { fab.classList.add('cart-fab-shake'); setTimeout(() => fab.classList.remove('cart-fab-shake'), 1000); }
    }
  }, 45000);
}
...
window.updateCartUI = function() {
  ...
  // Build items HTML
  let html = hasItems ? _shippingBar() : '';
    ...
    return `<div class="cart-item">
      <div class="_cit">
        ...
        <div class="_cbody">
          ...
          <div class="qty-row">
            <button class="qty-btn" onclick="window._updateQty(${i},-1)">−</button>
            <span class="qty-num">${qty}</span>
            <button class="qty-btn" onclick="window._updateQty(${i},1)">+</button>
          </div>
          <button class="save-later-btn" onclick="window._saveForLater(${i})">Save for later</button>
        </div>
  ...
  if (hasSaved) {
    ...
    html += savedForLater.map((item, i) => {
      const imgSrc = _getImg(item.productId);
      return `<div class="_saveditem"><div class="_savedimg">${imgSrc?`<img src="${imgSrc}">`:(item.category==='shoes'?'👟':'👕')}</div><div class="_savedname">${item.name}</div><button class="_m2c" onclick="window._moveToCart(${i})">Move to cart</button></div>`;
    }).join('') + '</div>';
  }
...
window._quickAdd = function(id, btn) {
  const p = (window.products||[]).find(p => p.id === id);
  if (!p || !p.available) return;
  const final = _finalPrice(p);
  const size = (p.sizes&&p.sizes[0])||'', color = (p.colors&&p.colors[0])||'';
  _merge({id: Date.now(), productId: p.id, name: p.name, brand: p.brand, price: final, size, color});
  window.saveCart(); window.updateCartUI();
  flyToCart(btn, id);
};

const _origQATC = window.quickAddToCart;
...
const _origATCDetail = window.addToCartFromDetail;
window.addToCartFromDetail = function() {
  ...
  _merge({id: Date.now(), productId: p.id, name: p.name, brand: p.brand, price: final, size, color});
  window.saveCart(); window.updateCartUI();
  const imgSrc = _getImg(id);
  _showToast(p, imgSrc);
  const fab = document.querySelector('.cart-fab');
  if (fab) { fab.classList.add('cart-fab-bounce'); setTimeout(() => fab.classList.remove('cart-fab-bounce'), 500); }
};

// ── RUN ON LOAD ─────────────────────────────────────────────────────────
window.updateCartUI();
_resetIdle();
console.log('✅ Nkrumah Ave cart upgrades loaded — all 9 features active');
